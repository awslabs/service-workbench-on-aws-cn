...
exports.getRegisterStudyForm = getRegisterStudyForm;
...
function getRegisterStudyForm() {
  var fields = ['account.id', 'account.name', 'account.description', 'account.contactInfo', 'account.mainRegion', 'bucket.name', 'bucket.region', 'bucket.sse', 'bucket.kmsArn', 'studies', 'studies[].id', 'studies[].name', 'studies[].folder', 'studies[].kmsArn', 'studies[].category', 'studies[].accessType', 'studies[].projectId', 'studies[].description', 'studies[].adminUsers'];
  var labels = {
  ...
  var placeholders = {
    ...
    'account.contactInfo': '(Optional) Type the contact information for the admins of this account. This information is purely for your convenience and it does not have any impact on the registration process.',
  ...
  return (0, _form.createFormSeparatedFormat)({
    fields: fields,
    labels: labels,
...
//# sourceMappingURL=RegisterStudyForm.js.map