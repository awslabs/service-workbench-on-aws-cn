...
function registerMenuItems(itemsMap, _ref) {
  ...
  var canCreateWorkspaces = _lodash["default"].get(appContext, 'userStore.user.capabilities.canCreateWorkspace');

  var canViewDashboard = _lodash["default"].get(appContext, 'userStore.user.capabilities.canViewDashboard');

  var items = new Map([].concat(_toConsumableArray(_lodash["default"].filter(_toConsumableArray(itemsMap), function (item) {
    ...
    title: 'Workspaces',
    icon: 'server',
    shouldShow: canCreateWorkspaces
  }]]));
  return items;
}
