...
async function createWorkflowTemplateDraft({
  isNewTemplate,
  templateId,
  templateTitle
}) {
...
async function updateWorkflowTemplateDraft(draft) {
  return httpApiPut(`api/workflow-templates/drafts/${encodeURIComponent(draft.id)}`, {
    data: draft
  });
}

async function publishWorkflowTemplateDraft(draft) {
  return httpApiPost('api/workflow-templates/drafts/publish', {
    data: draft
  });
}
...
async function updateWorkflowDraft(draft) {
  return httpApiPut(`api/workflows/drafts/${encodeURIComponent(draft.id)}`, {
    data: draft
  });
}

async function publishWorkflowDraft(draft) {
  return httpApiPost('api/workflows/drafts/publish', {
    data: draft
  });
}
...
async function listWorkflowInstancesByStatus({
  status,
  data
}) {
  return httpApiPost(`api/workflows/instances/status/${status}`, {
    data
  });
}
...
async function getWorkflowInstance(workflowId, workflowVer, instanceId) {
  return httpApiGet(`api/workflows/${encodeURIComponent(workflowId)}/v/${workflowVer}/instances/${encodeURIComponent(instanceId)}`);
}

async function triggerWorkflow(workflowId, workflowVer, data) {
  return httpApiPost(`api/workflows/${encodeURIComponent(workflowId)}/v/${workflowVer}/trigger`, {
    data
  });
}
...
export { getWorkflowTemplates, getWorkflowTemplate, getWorkflowTemplateDrafts, createWorkflowTemplateDraft, updateWorkflowTemplateDraft, publishWorkflowTemplateDraft, deleteWorkflowTemplateDraft, getStepTemplates, getWorkflows, getWorkflowDrafts, createWorkflowDraft, updateWorkflowDraft, publishWorkflowDraft, deleteWorkflowDraft, getWorkflow, listWorkflowInstancesByStatus, getWorkflowInstances, getWorkflowInstance, triggerWorkflow, getWorkflowAssignments };
