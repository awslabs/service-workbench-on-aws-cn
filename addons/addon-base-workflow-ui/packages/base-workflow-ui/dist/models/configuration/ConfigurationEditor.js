...
const ConfigurationEditor = types.model('ConfigurationEditor', {
  currentSectionIndex: 0,
  // IMPORTANT section index start from 0 not 1
  review: false,
  inputManifest: types.maybe(InputManifest),
  configuration: types.optional(types.map(types.union(types.null, types.undefined, types.integer, types.number, types.boolean, types.string)), {}),
  ...
  const superCleanup = self.cleanup; // If the value of a form field is an object, then make the value a json string instead
  ...
  get definedConfigList() {
    ...
    _.forEach(inputEntries, entry => {
      ...
      if (!_.isUndefined(value)) list.push({ ...entry,
        value
      });
    });
