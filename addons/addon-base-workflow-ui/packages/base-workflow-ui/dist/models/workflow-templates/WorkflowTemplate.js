...
const PropsOverrideOption = types.model('PropsOverrideOption', {
  ...
  get overrideSummaryRows() {
    ...
    const result = [{
      title: titles.steps,
      allowed: canOverride('steps'),
      name: 'steps'
    }, {
      title: titles.instanceTtl,
      allowed: canOverride('instanceTtl'),
      name: 'instanceTtl'
    }, {
      title: titles.runSpecSize,
      allowed: canOverride('runSpecSize'),
      name: 'runSpecSize'
    }, {
      title: titles.runSpecTarget,
      allowed: canOverride('runSpecTarget'),
      name: 'runSpecTarget'
    }, {
      title: titles.title,
...
const RunSpec = types.model('RunSpec', {
  size: '',
  target: ''
}).views(self => ({
...
const WorkflowTemplateVersion = types.model('WorkflowTemplateVersion', {
  id: '',
  v: types.number,
  ...
  setInstanceTtl(value) {
    ...
    if (_.isString(value)) {
      ...
      if (_.isNaN(parsed)) answer = null;else answer = parsed;
...
const WorkflowTemplate = types.model('WorkflowTemplate', {
  id: types.identifier,
