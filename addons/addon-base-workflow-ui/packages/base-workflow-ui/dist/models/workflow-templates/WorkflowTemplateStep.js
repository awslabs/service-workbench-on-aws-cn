...
const PropsOverrideOption = types.model('PropsOverrideOption', {
  ...
  get overrideSummaryRows() {
    ...
    const result = [{
      title: titles.title,
      allowed: canOverride('title'),
      name: 'title'
    }, {
      title: titles.desc,
      allowed: canOverride('desc'),
      name: 'desc'
    }, {
      title: titles.skippable,
      allowed: canOverride('skippable'),
      name: 'skippable'
    }];
...
const ConfigOverrideOption = types.model('ConfigOverrideOption', {
  allowed: types.optional(types.array(types.string), [])
}).actions(self => ({
  setAllowed(allowed = []) {
    self.allowed.replace(allowed);
...
const WorkflowTemplateStep = types.model('WorkflowTemplateStep', {
  id: '',
  stepTemplateId: '',
  ...
  defaults: types.optional(types.map(types.union(types.null, types.undefined, types.integer, types.number, types.boolean, types.string)), {})
}).actions(self => ({
  afterCreate() {
    if (_.isEmpty(self.id)) console.warn(`There is no id provided for this workflow template step`, getSnapshot(self));
  },

  setDesc(desc) {
    if (desc === self.stepTemplate.desc) self.desc = undefined;else self.desc = desc;
  },

  setTitle(title) {
    if (title === self.stepTemplate.title) self.title = undefined;else self.title = title;
  },
  ...
  get configSummaryRows() {
    ...
    const inputManifest = self.stepTemplate.inputManifest; // We use 'additional' to keep track of entries that was not part of the inputManifest but yet there is a key in 'configs' for it.
