...
import { getWorkflowTemplateDrafts, createWorkflowTemplateDraft, updateWorkflowTemplateDraft, publishWorkflowTemplateDraft, deleteWorkflowTemplateDraft } from '../../../helpers/api';
import WorkflowTemplateDraft from './WorkflowTemplateDraft'; // ==================================================================
// WorkflowTemplateDraftsStore
// ==================================================================

const WorkflowTemplateDraftsStore = BaseStore.named('WorkflowTemplateDraftsStore').props({
  drafts: types.optional(types.map(WorkflowTemplateDraft), {}),
  tickPeriod: 900 * 1000 // 15 minutes

}).actions(self => {
  // save the base implementation of cleanup
  const superCleanup = self.cleanup; // private
  ...
  return {
    async doLoad() {
      const drafts = await getWorkflowTemplateDrafts(); // We try to preserve existing drafts data and merge the new data instead
    ...
    async createDraft({
      ...
      const draft = await createWorkflowTemplateDraft({
        isNewTemplate,
        templateId,
        templateTitle
      });
...
//# sourceMappingURL=WorkflowTemplateDraftsStore.js.map