...
const WorkflowTemplateDraftEditor = types.model('WorkflowTemplateDraftEditor', {
  draftId: '',
  currentPage: 0,
  // there are only two pages, one for meta editing and one for steps editing
  numPages: 3,
  ...
  return {
    ...
    nextPage() {
      if (self.currentPage < self.numPages - 1) self.currentPage += 1;else self.currentPage = self.numPages - 1;
      return self.currentPage;
    },

    previousPage() {
      if (self.currentPage > 0) self.currentPage -= 1;else self.currentPage = 0;
      return self.currentPage;
    },
    ...
    getStepEditor(step) {
      ...
      const entry = self.stepEditors.get(stepId) || WorkflowTemplateStepEditor.create({
        stepId
      }, getEnv(self));
    ...
    async update(draft) {
      const updatedDraft = await self.workflowTemplateDraftsStore.updateDraft(draft); // The following code is not the greatest idea, but okay for this scenario, the correct approach would have
    ...
    async publish(draft) {
      const result = await self.workflowTemplateDraftsStore.publishDraft(draft); // Remove the wizard from the session store, if there were no errors
...
function getWorkflowTemplateDraftEditor(draftId) {
  ...
  const entry = sessionStore.map.get(id) || WorkflowTemplateDraftEditor.create({
    draftId
  }, globals);
