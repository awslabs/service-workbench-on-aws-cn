...
import { PropsOverrideOption, ConfigOverrideOption, supportedPropsOverrideKeys } from '../workflow-templates/WorkflowTemplateStep';
const titles = {
  title: 'Title',
  desc: 'Description',
...
const WorkflowStep = types.model('WorkflowStep', {
  id: '',
  stepTemplateId: '',
  ...
  configs: types.optional(types.map(types.union(types.null, types.undefined, types.integer, types.number, types.boolean, types.string)), {})
}).actions(self => ({
  afterCreate() {
  ...
  get configSummaryRows() {
    ...
    const inputManifest = stepTemplate.inputManifest; // We use 'additional' to keep track of entries that was not part of the inputManifest but yet there is a key in 'configs' for it.
