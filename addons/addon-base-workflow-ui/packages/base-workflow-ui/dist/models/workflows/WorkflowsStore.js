...
const WorkflowsStore = BaseStore.named('WorkflowsStore').props({
  ...
  return {
    async doLoad() {
      ...
      const workflows = toWorkflows(versions); // we try to preserve existing workflow versions data and merge the new data instead
    ...
    getWorkflowStore: workflowId => {
      ...
      if (!entry) {
        ...
        self.workflowStores.set(workflowId, WorkflowStore.create({
          workflowId
        }));
    ...
    self.workflows.forEach(wf => {
      ...
      wf.versions.forEach(wfv => {
        result.push({
          ...
          text: wfv.v === latestWfVersion ? `${wf.id} (latest)` : `${wf.id} (v${wfv.v})` // content:
...
//# sourceMappingURL=WorkflowsStore.js.map