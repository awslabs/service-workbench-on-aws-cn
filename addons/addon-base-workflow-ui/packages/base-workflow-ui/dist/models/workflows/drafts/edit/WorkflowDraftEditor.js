...
const WorkflowDraftEditor = types.model('WorkflowDraftEditor', {
  draftId: '',
  currentPage: 0,
  ...
  return {
    ...
    getStepEditor(step) {
      ...
      const entry = self.stepEditors.get(stepId) || WorkflowStepEditor.create({
        stepId
      }, getEnv(self));
    ...
    async update(draft) {
      const updatedDraft = await self.workflowDraftsStore.updateDraft(draft); // The following code is not the greatest idea, but okay for this scenario, the correct approach would have
    ...
    async publish(draft) {
      const result = await self.workflowDraftsStore.publishDraft(draft); // Remove the editor from the session store, if there were no errors
...
function getWorkflowDraftEditor(draftId) {
  ...
  const entry = sessionStore.map.get(id) || WorkflowDraftEditor.create({
    draftId
  }, globals);
