...
Service Workbench 可以在内部托管研究数据集，并提供在部署Service Workbench的AWS账户之外的S3存储桶中的外部研究数据的访问权限。数据源页面使管理员能够配置和管理这些外部研究数据。

外部研究数据账户必须向 Service Workbench 提供访问包含外部研究数据的特定 S3 存储桶和路径的权限。这些权限是使用Service Workbench程序生成的 AWS CloudFormation 模板在外部账户中创建的。

数据源页面列出了已在 Service Workbench 中注册的外部研究数据。外部研究数据也出现在具有权限的用户的研究数据页面上。

## 使用 Service Workbench 注册外部研究数据 ##
...
5. 对于**Project**，选择一个项目。
6. 选择**类型**。有关详细信息，请参阅 [创建研究数据](/zh/user_guide/sidebar/common/studies/creating_a_study)。
7. 对于 **Access**，选择 **Read Only** 或 **Read/Write**。如果是组织的研究数据，则可以在研究页面上按用户分配适当的权限。
8. 对于 **Description**，输入研究项目详细信息。
9. 对于 **Study KMS ARN**，输入值（如果适用）。
10. 对于**Admin**，如果是组织研究数据，请为研究数据选择一个或多个管理员。这可以稍后在 **Studies** 页面上进行编辑。
20. 单击**保存并继续**。
...
如果外部研究帐户是首次加入 Service Workbench，则选择 *Create Stack* 选项。该应用程序将生成一个适合首次加入的新 CloudFormation 模板。
